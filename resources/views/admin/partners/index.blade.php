...
@section('page_subtitle', 'Atur partner dan sponsor acara Anda di sini.')
@section('content')

{{-- Search Form --}}
<div class="mb-6 flex gap-4">
    <form action="{{ route('admin.partners.index') }}" method="GET" class="flex-1 flex gap-4">
        <div class="flex-1">
            <input type="text" name="search" value="{{ old('search', $search ?? '') }}"
                   placeholder="Cari partner "
                   class="w-full px-5 py-3 bg-white border-2 border-slate-100 rounded-2xl
                ...
                @forelse($partners as $index => $partner)
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="px-8 py-6">
                            ...
                            @else
                                <div class="w-16 h-16 rounded-xl bg-slate-200 flex items-center justify-center text-slate-400 text-xs">
                                    No Logo
                                </div>
                            @endif
                        </td>
