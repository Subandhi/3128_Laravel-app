...
@section('page_subtitle', 'Perbarui nama kategori.')
    ...
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $category->name) }}"
                ...
                placeholder="Contoh: Music, Sport, Technology"
                required>
        ...
        <div>
            ...
            <input type="text" name="slug" value="{{ old('slug', $category->slug) }}"
                class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl
                focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium"
                placeholder="Contoh: music, sport, technology (hanya huruf kecil, angka, dan tanda hubung)"
                required>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" value="{{ $category->created_at->format('d M Y H:i') }}"
            ...
            <div>
                ...
                <input type="text" value="{{ $category->updated_at->format('d M Y H:i') }}"
                    class="w-full px-5 py-4 bg-slate-100 border-2 border-slate-100 rounded-2xl text-slate-500"
                    disabled>
            </div>
        </div>
